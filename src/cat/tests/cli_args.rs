...
fn parse_with_single_input() {
    ...
    let args = vec!["cat".to_string(), FILE_NAME.to_string()];
    let ret = cli_args::parse(args.clone()).unwrap();

    assert_eq!(
        ret.inputs().contains(&cli_param::Input::File(FILE_NAME.to_string())),
        true);
    assert_eq!(ret.inputs().len(), args.len() - 1);
    assert_eq!(ret.flags().is_empty(), true)
}
...
fn parse_with_multiple_inputs() {
    ...
    for arg in &args[1..] {
        assert_eq!(
            ret.inputs().contains(&cli_param::Input::File(arg.to_string())),
            true);
    }
